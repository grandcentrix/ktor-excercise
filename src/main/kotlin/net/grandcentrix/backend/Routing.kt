...
fun Application.configureRouting(youtubeManager: YouTubeManagerInterface, playlistManager: PlaylistManager) {
    routing {
        index(youtubeManager, playlistManager)
...
private fun Routing.addVideo(youtubeManager: YouTubeManagerInterface) {
    post("/addVideo") {
        ...
        val (status,response) = youtubeManager.saveYouTubeLinks(newVideoUrl)
        call.respond(status, response)

    }
}

private fun Routing.index(
    youtubeManager: YouTubeManagerInterface,
                    ...
                    youtubeManager.getYoutubeLinks().forEachIndexed { index, videoInfo ->
                        li {
                            ...
                            a(
                                href = videoUrl,
                                target = "_blank"
                            ) { +if (videoInfo.customName.isNotEmpty()) videoInfo.customName else videoUrl }
